...
// @license 	WTFPLv2
// @encoding    utf-8
// @include	http://*.ucnan.org.ua/*
// @include	https://*.ucnan.org.ua/*
// @include	http://*.ucnan.to/*
// @include	https://*.ucnan.to/*

// ==/UserScript==

function init_prew_posts ()
{
...
config['limit_preview_by_replies']=true;
...
function ajax_query() {
  ...
  function make_connection(method,url,fn) {
    ...
    try {
      ...
      handle.setRequestHeader('Cookie','js_bot=bot;');
      handle.send(null);
    }
...
function getBoardEngineType(obj) {
  ...
  if (RegExp('^https?://(|[^/]*\.)dobrochan.ru').test(document.URL)) {
    type='hanabira';
  }
  return type;
}
...
function initPostPreview(e) {
  ...
  post['isregularlink']=RegExp('https?://'+post['host']).test(post['href']);
  post['isabbrev']=(post['link'].parentNode) ? (post['link'].parentNode.className=='abbrev') : false;
  post['iserr']=RegExp('https?://'+post['host']+'/cgi-bin/').test(post['url']);
  post['isindomain']=RegExp('^https?://(|[^/]*[.])'+post['host']).test(post['text']);
  switch (config['system']['engine_type']) {
    case 'wakaba':
    case 'wakaba3':
    case 'hanabira':
      regexp['href']='(^|'+post['host']+')/((.*)/|)(([0-9]*)\.[xs]{0,1}html|)(|[?#i]{0,3}([0-9]*))$';
      regexp['url']='https?://'+post['host']+'/((.*)/res/[0-9]*\.[xs]{0,1}html[?i#]{0,3}[0-9]*|(.*)[/]+[^/]*|[^/]*|)$';
      break;
  }
...
function rollPostPreview(post) {
  ...
  var parentelem;
  preview['leftpos'] = findPosX(post['link']);
  preview['toppos']  = findPosY(post['link']);
  ...
  page_protocol = (window.location.protocol == "https:") ? "https://" : "http://";
  preview['url']=RegExp('^https?://[^/]*((.*)[#?]|(.*))').exec(post['href']);
  preview['url']=(preview['url'][2]) ? preview['url'][2] : preview['url'][1];
  preview['url']=page_protocol+post['host']+preview['url'];
...
function bakePostPreview(post,preview) {
  ...
  for(var i=0;i<links.length;i++)
    if (RegExp('(^/'+post['board']+'/|^/)').test(links[i].href)) 
      links[i].href=page_protocol+post['host']+links[i].href;
...
window.addEventListener("load",init_prew_posts,false);
/*
     FILE ARCHIVED ON 09:05:01 Apr 25, 2016 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 15:29:56 Apr 01, 2020.
...
/*
playback timings (ms):
  LoadShardBlock: 2778.155 (3)
  CDXLines.iter: 15.085 (3)
  exclusion.robots: 0.344
  captures_list: 2839.852
  esindex: 0.014
  exclusion.robots.policy: 0.329
  RedisCDXSource: 42.486
  PetaboxLoader3.resolve: 3353.343 (3)
  load_resource: 745.311
  PetaboxLoader3.datanode: 145.811 (4)
*/